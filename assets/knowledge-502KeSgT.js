import{c as A,z as B,k as f,s as k}from"./index-CWYTB3B6.js";/**
 * @license lucide-vue-next v0.294.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const Pn=A("HeartIcon",[["path",{d:"M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z",key:"c3ymky"}]]),E=`---\r
...
4. **缓存**: 使用 contenthash 实现长效缓存`,R=`---\r
...
- **简单场景**: 使用 margin: 0 auto（水平居中）`,V=`---\r
...
| 可折叠内容 | height: 0 + overflow: hidden |`,X=`---\r
title: "重排与重绘"\r
category: "CSS"\r
...
- **代码优化**: 将对象操作统一为函数式调用，便于组合和复用`,G=`---\r
title: "this 绑定机制"\r
category: "JavaScript"\r
...
3. **设置 CSP**: 配置内容安全策略\r
4. **使用安全库**: 使用 DOMPurify 等安全库处理富文本`,tn=`---\r
title: "职业规划回答技巧"\r
category: "Other"\r
...
- **对象类型**: 使用 \`reactive\`\r
- **复杂计算**: 使用 \`computed\`\r
- **数据监听**: 使用 \`watch\` 或 \`watchEffect\``,pn=`---\r
...
- **复杂逻辑**: 使用 Composition API 的 composables`,un=`---\r
...
`,mn=/^---\s*\n([\s\S]*?)\n---\s*\n/;function gn(n){const r={};return n.split(`
`).forEach(e=>{const s=e.match(/^(\w+):\s*(.+)$/);if(s){const a=s[1];let t=s[2].trim();t.startsWith('"')&&t.endsWith('"')?t=t.slice(1,-1):t.startsWith("[")&&t.endsWith("]")&&(t=JSON.parse(t)),r[a]=t}}),r}function fn(n){const r=n.match(mn);let o={title:"",category:"",tags:[],difficulty:"medium",filePath:"",lastModified:""},e=n;if(r){const a=gn(r[1]);o={title:a.title||"",category:a.category||"",tags:a.tags||[],difficulty:a.difficulty||"medium",filePath:"",lastModified:""},e=n.slice(r[0].length)}const s=hn(e);return{meta:o,content:e,toc:s}}function hn(n){const r=/^(#{2,3})\s+(.+)$/gm,o=[];let e;for(;(e=r.exec(n))!==null;){const s=e[1].length,a=e[2].trim(),t=a.toLowerCase().replace(/[^a-z0-9]+/g,"-");o.push({id:t,text:a,level:s})}return o}function vn(n){const r=n.split(/\r?\n/),o=[];let e="";return r.forEach(s=>{const a=s.match(/^###\s+(.+)$/);if(a){e=a[1].trim();return}const t=s.match(/^\|\s*\[(.+?)\]\((.+?)\)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*$/);if(t){const d=t[1],p=t[2],u=t[3].trim(),y={简单:"easy",中等:"medium",高:"hard"}[u]||"medium",b=t[4].split(",").map(h=>h.trim()).filter(Boolean);o.push({title:d,category:e,tags:b,difficulty:y,filePath:p,lastModified:""})}}),o}function yn(n,r,o){if(!r.trim())return[];const e=r.toLowerCase(),s=[];return n.forEach(a=>{let t=0;const d=[];a.title.toLowerCase().includes(e)&&(t+=50,d.push({text:a.title,highlight:w(a.title,e)})),a.tags.forEach(u=>{u.toLowerCase().includes(e)&&(t+=20,d.push({text:u,highlight:w(u,e)}))});const p=o.get(a.filePath);if(p){const u=p.toLowerCase();if(u.includes(e)){t+=30;const v=u.indexOf(e),y=Math.max(0,v-30),b=Math.min(p.length,v+e.length+30),h=p.slice(y,b);d.push({text:h,highlight:w(h,e)})}}t>0&&s.push({document:a,matches:d,score:t})}),s.sort((a,t)=>t.score-a.score)}function w(n,r){const o=new RegExp(`(${r})`,"gi");return n.replace(o,'<mark class="bg-accent-500 text-white px-0.5 rounded">$1</mark>')}const j="knowledge_base_favorites";function x(){try{const n=localStorage.getItem(j);return n?JSON.parse(n):[]}catch{return[]}}function bn(n){const r=x();r.some(e=>e.filePath===n.filePath)||(r.push(n),localStorage.setItem(j,JSON.stringify(r)))}function xn(n){const r=x().filter(o=>o.filePath!==n);localStorage.setItem(j,JSON.stringify(r))}function _(n){return x().some(r=>r.filePath===n)}const Sn="FaceNoteReadStats",m="readStats",wn=1;function C(){return new Promise((n,r)=>{const o=indexedDB.open(Sn,wn);o.onupgradeneeded=()=>{const e=o.result;e.objectStoreNames.contains(m)||e.createObjectStore(m,{keyPath:"filePath"})},o.onsuccess=()=>n(o.result),o.onerror=()=>r(new Error("IndexedDB 打开失败"))})}async function jn(){try{const n=await C();return await new Promise((r,o)=>{const a=n.transaction(m,"readonly").objectStore(m).getAll();a.onsuccess=()=>r(a.result),a.onerror=()=>o(new Error("读取阅读统计失败"))})}catch{return[]}}async function Cn(n){try{const r=await C();return await new Promise((o,e)=>{const t=r.transaction(m,"readonly").objectStore(m).get(n);t.onsuccess=()=>o(t.result??void 0),t.onerror=()=>e(new Error("读取阅读统计失败"))})}catch{return}}async function kn(n){const r=await C(),o=await Cn(n),e=((o==null?void 0:o.totalReadCount)??0)+1;return await new Promise((s,a)=>{const p=r.transaction(m,"readwrite").objectStore(m).put({filePath:n,totalReadCount:e,lastReadTimestamp:Date.now()});p.onsuccess=()=>s(e),p.onerror=()=>a(new Error("保存阅读统计失败"))})}const Tn=B("knowledge",()=>{const n=f([]),r=f(new Map),o=f(null),e=f(null),s=f([]),a=f(!1),t=f(new Map);async function d(){const i=await jn(),l=new Map;i.forEach(c=>l.set(c.filePath,c.totalReadCount)),t.value=l}function p(i){return t.value.get(i)??0}async function u(i){const l=await kn(i);t.value.set(i,l)}const v=k(()=>{const i=new Map;n.value.forEach(c=>{i.set(c.category,(i.get(c.category)||0)+1)});const l={"JavaScript/TypeScript":"Code",CSS:"Palette",Vue:"Box",React:"Atom",Network:"Globe",Browser:"Monitor",Build:"Package",Other:"FileQuestion",AI:"Brain",Security:"Shield",Performance:"Zap",SystemDesign:"Network",Algorithms:"Binary",Architecture:"Layers"};return Array.from(i.entries()).map(([c,g])=>({name:c,label:c,icon:l[c]||"FileText",count:g}))}),y=k(()=>{let i=n.value;return o.value&&(i=i.filter(l=>l.category===o.value)),e.value&&(i=i.filter(l=>l.difficulty===e.value)),i.sort((l,c)=>l.title.localeCompare(c.title))});async function b(){a.value=!0;try{const i=Object.assign({"../knowledgeBase/AI/prompt-engineering.md":E,"../knowledgeBase/Browser/url-lifecycle.md":I,"../knowledgeBase/Build/build-optimization.md":O,"../knowledgeBase/Build/webpack-chunk.md":H,"../knowledgeBase/CSS/bfc.md":R,"../knowledgeBase/CSS/element-centering.md":N,"../knowledgeBase/CSS/flexbox-layout.md":V,"../knowledgeBase/CSS/hide-element.md":D,"../knowledgeBase/CSS/reflow-repaint.md":X,"../knowledgeBase/JavaScript/closure.md":F,"../knowledgeBase/JavaScript/debounce-throttle.md":J,"../knowledgeBase/JavaScript/deep-shallow-copy.md":L,"../knowledgeBase/JavaScript/event-loop.md":z,"../knowledgeBase/JavaScript/i18n.md":$,"../knowledgeBase/JavaScript/nodejs-capabilities.md":W,"../knowledgeBase/JavaScript/reflect.md":q,"../knowledgeBase/JavaScript/this-binding.md":G,"../knowledgeBase/JavaScript/var-let-const.md":U,"../knowledgeBase/JavaScript/virtual-list.md":K,"../knowledgeBase/Network/cors.md":Y,"../knowledgeBase/Network/http-cache.md":Q,"../knowledgeBase/Network/http-protocol.md":Z,"../knowledgeBase/Network/mcp-principle.md":nn,"../knowledgeBase/Network/sse-polling.md":rn,"../knowledgeBase/Network/xss-attack.md":en,"../knowledgeBase/Other/career-plan.md":tn,"../knowledgeBase/Other/git-operations.md":on,"../knowledgeBase/Other/resign-reasons.md":an,"../knowledgeBase/Other/self-introduction.md":sn,"../knowledgeBase/Vue/component-communication.md":cn,"../knowledgeBase/Vue/reactivity-principle.md":ln,"../knowledgeBase/Vue/vue-mixins.md":pn,"../knowledgeBase/Vue/vue2-vs-vue3.md":un,"../knowledgeBase/index.md":dn}),l=i["../knowledgeBase/index.md"]||"";n.value=vn(l),n.value.forEach(c=>{const g=`../knowledgeBase/${c.filePath}`,S=i[g];S?r.value.set(c.filePath,S):r.value.set(c.filePath,"")}),s.value=x()}catch(i){console.error("Failed to load documents:",i)}finally{a.value=!1}}function h(i){const l=r.value.get(i);if(!l)return null;const c=fn(l);c.meta.filePath=i;const g=n.value.find(S=>S.filePath===i);return g&&(c.meta.category=g.category,c.meta.lastModified=g.lastModified),c}function P(i){return yn(n.value,i,r.value)}function T(i){_(i.filePath)?xn(i.filePath):bn(i),s.value=x()}function M(i){return _(i)}return{documents:n,contentMap:r,currentCategory:o,currentDifficulty:e,favorites:s,isLoading:a,readStatsMap:t,categories:v,filteredDocuments:y,loadDocuments:b,loadReadStats:d,getReadCount:p,recordRead:u,getDocumentContent:h,search:P,toggleFavorite:T,isDocFavorite:M}});export{Pn as H,Tn as u};
